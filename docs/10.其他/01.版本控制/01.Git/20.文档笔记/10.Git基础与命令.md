...
git push origin --tag # 一次性推送所有不在远程仓库上的标签
```
...
注意上述命令并不会从任何远程仓库中移除这个标签，你必须用 `git push <remote> :refs/tag/<tagname>` 来更新你的远程仓库：

第一种变体是 `git push <remote> :refs/tag/<tagname>` ：

```sh
$ git push origin :refs/tag/v1.4-lw
